const d = document;
const $table = d.querySelector('.crud-table--mesas'),
      $template = d.getElementById('crud-template').content,
      $formBuscarProductos = d.forms["buscarMesa"],
      $fragment = d.createDocumentFragment(),
      $inputform = $formBuscarProductos.elements["vm"];
...
$inputform.addEventListener('keyup',(e) =>{
    ...
    ajax({
        url: `http://localhost:8085/TE_restaurante/MesaControlador?action=search&vm=${$inputform.value}`,
        exito:(res) => {
            console.log(res)
            res.forEach(el => {
...
d.addEventListener('DOMContentLoaded', (e)=>{
    ...
    ajax({
        url: 'http://localhost:8085/TE_restaurante/MesaControlador?action=search&vm=',
        exito:(res) => {
            console.log(res)
            res.forEach(el => {
                $template.querySelector(".cu-id").textContent = el.id;
                $template.querySelector(".cu-nombre").textContent = el.nombre;
                $template.querySelector(".cu-encargado").textContent = el.encargado;
                $template.querySelector(".cu-ocupada").textContent = el.ocupada;
                $template.querySelector(".cu-edit").href = `ProductoControlador?action=edit&id=${el.id}`;
